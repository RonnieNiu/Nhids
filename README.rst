...
定义告警级别为：低；如果此时agent_ip 与 NIDS dest_ip 也相等，说明此ip产生的告警，在HIDS和NIDS里攻击源一致，告警级别定义为：高；

2.如果1不满足，则判断hids agentip 是否与nids srcip相等，如果相等，说明此agent不但在hids层面产生了告警 ，同时作为srcip 在nids
层面发起了攻击，常见如自己被沦陷，然后作为跳板横向渗透，认为是一个可关联的告警，定义告警级别为；中；

3.如果1和2都不满足，则判断hids agentip 是否与nids destip
是否相等，如果相等，说明此agent受到了不同源ip的攻击，也可以作为一个可关联的告警，定义告警级别为：低；
